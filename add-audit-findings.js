...
// Replace each question carefully
Object.entries(auditUpdates).forEach(([questionNumber, updates]) => {
  // More specific pattern that captures the entire object
  const escapedNum = questionNumber.replace(/\./g, '\\.');
  const pattern = new RegExp(
    `({\\s*section:\\s*'ILO',\\s*[\\s\\S]*?questionNumber:\\s*'${escapedNum}',\\s*question:\\s*'[^']*',\\s*progress:\\s*\\d+,\\s*completed:\\s*\\d+,\\s*total:\\s*\\d+,?)(\\s*})`,
    'm'
  );
  ...
  if (pattern.test(content)) {
    ...
    const replacement = `$1,
    auditFindings: '${escapedUpdates.auditFindings}',
    commonProblems: '${escapedUpdates.commonProblems}',
    advice: '${escapedUpdates.advice}',
    reminder: '${escapedUpdates.reminder}'$2`;

    content = content.replace(pattern, replacement);
    console.log(`✅ Updated ${questionNumber}`);
  } else {
    console.log(`❌ Pattern not found for ${questionNumber}`);
  }
});

fs.writeFileSync(seedPath, content, 'utf-8');
console.log('\n✅ All audit findings added successfully!');
